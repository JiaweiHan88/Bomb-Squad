...
3. **Sequential play with the resting team able to spectate.** Both teams play the **same rounds sequentially** (the current V1 actually arms both teams' bombs in the same `ROUND_START`, i.e. concurrently — see the Cross-Story Seam note; "sequential, not parallel" is honoured at the *relay-structure* level: there is one shared round number and one between-rounds gate, never two independent relays). The resting team (one whose `relayOrder` is exhausted while the other is still equalising) is **able to spectate** the active team — no client is stranded or shown a dead surface.
...
**The "sequential, not parallel" AC vs. the V1 reality:** the current V1 `ROUND_START` arms a timer + emits `BOMB_INIT` for **every** team in `round.defusers` *simultaneously* (`sessionHandlers.ts` ~1050–1115). So both teams' bombs run concurrently within a round. The "sequential relay" requirement (FR43) is honoured at the **relay-structure** level — one shared `roundNumber`, one all-teams-resolved between-rounds gate (8.6), never two independent relays. Do NOT try to serialise the two teams' bombs within a round; that is not this story's scope and would regress 8.5/8.6. The equalisation round is the one case where only ONE team has an armed bomb (the resting team is absent from `round.defusers`).
