...
- [~] **Task 10 — Human verification (per project rule [[human-verification-ac-rule]]) — DEFERRED (2026-06-17)**
  - [ ] **DEFERRED by Jay's explicit request:** story closed to `done` ahead of interactive verification; the human-verification gate is consciously deferred to a later session (recorded in `deferred-work.md`). Automated coverage is complete and green (813 tests, typecheck clean). When run later: Jay verifies interactively on the full Docker stack (server on plain `tsx`, **not** `tsx watch`, per [[timer-verification-tsx-watch-gotcha]]) at `https://localhost`: run a round to resolution (defuse and/or timeout) with two teams → all players land on the scoreboard preview only after BOTH teams finish (never mid-round); standings show cumulative + per-round times and the leader; the next round does not auto-start; the Facilitator's "Start next round" advances to Preparation showing the NEXT Defuser per team; cancel from that Preparation returns to the scoreboard (not the lobby) with the same standings; a browser refresh during between-rounds re-renders the scoreboard (reconnect-safe). Record the observed result here. **Not done until this is filled in.**
...
- **Human verification: PENDING** (Task 10). Automated coverage is complete and green, but per [[human-verification-ac-rule]] the story is not "done" until Jay's observed result is recorded here. Run the server WITHOUT `tsx watch` ([[timer-verification-tsx-watch-gotcha]]) so the in-memory expiry wake survives.
