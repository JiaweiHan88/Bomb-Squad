...
#### Follow-ups discovered during Story 4.7 interactive verification (2026-06-13)

Story 4.7 wired the `MODULE_INTERACT` handler to `onBombDefused` / `onThirdStrike`, making the **defuse** and **DETONATED** resolution paths interactively runnable for the first time. Jay's hands-on run confirmed both (and 4.7 fixed the client's terminal-strike count broadcast so a 3rd-strike loss now labels DETONATED, not TIME EXPIRED — closing the limitation noted in Task 5 / `bindServerEvents`). Two resolution-presentation defects surfaced that belong to this story (8.5), not 4.7's ACs:

- [ ] [Review][Follow-up] **Timer LCD keeps counting after a round resolves.** On `BOMB_DEFUSED`/`BOMB_EXPLODED` the client sets the banner but never stops the timer LCD, so it keeps extrapolating behind the overlay (visible on an interactive defuse/explosion *before* natural expiry — the timeout path masked it by being at 0). Fix: freeze the LCD while `resolution !== null` (e.g. early-out in `TimerLcd`'s `useFrame`, or clear/freeze the timer on resolution). [apps/client/src/scenes/TimerLcd.tsx + bindServerEvents/gameStore]
- [ ] [Review][Follow-up] **ResolutionBanner dim overlay flickers on browser scroll.** The semi-transparent banner (`bg-red-900/60`) compositing over the WebGL canvas repaints on scroll. Likely a layout/compositing issue (stray page scroll + transparent overlay over the canvas). Investigate AppShell height/overflow and overlay compositing (`will-change`/own layer). [apps/client/src/ui/ResolutionBanner.tsx + AppShell]
...
- `apps/client/src/ui/ResolutionBanner.tsx` — NEW: rendering-only result-banner overlay (holds + tint + interim post-round surface + Epic-10 SFX placeholder); review: `held` resets on every outcome change; 4.7 follow-up: overlay layers GPU-promoted (`transform-gpu`) to stop scroll-repaint flicker.
- `apps/client/src/scenes/TimerLcd.tsx` — 4.7 follow-up: freeze the LCD while `resolution !== null` (hold the first-resolved-frame remaining instead of extrapolating via `serverNow()`).
