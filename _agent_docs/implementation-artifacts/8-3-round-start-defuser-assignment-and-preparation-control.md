...
- [x] Task 7 — Gates & verification (AC: all)
  ...
  - [x] **Jay verifies interactively** (three browser sessions against the Docker/Caddy stack — facilitator + two players on one team): host opens prep from the lobby, joiner sees the prep surface for their role, host starts the round, joiner routed to their active surface, defuser sees the bomb scene. Observed result recorded in Completion Notes (2026-06-13) — confirmed, including the three follow-up fixes below.
...
- **AC-6 — human verification (Jay, 2026-06-13):** confirmed against the Docker/Caddy stack (`https://localhost`, prod build — a stronger path than the spec's `pnpm dev` since it exercises the real same-origin Caddy `/socket.io/*` proxy) with three browser sessions = facilitator + two players assigned to one team. Observed: facilitator opens prep → both players see role-gated prep surfaces (upcoming defuser shown, "2–5 min" guidance, no countdown) → facilitator starts the round → the rotation-selected player is committed Defuser and sees the bomb scene; the other becomes Expert on the ManualViewer; facilitator routes but is never team-assigned. All as designed.
- **Follow-up fixes from that verification session (commit `e921310`):** three preparation-flow defects Jay surfaced were fixed and re-verified live:
  - **① Prep-open population guard** — previously `PREPARATION_OPEN` could open prep with no player on any team, stranding the facilitator (`ROUND_START` then refuses `NO_POPULATED_TEAM` and, pre-③, prep had no exit). Added pure `hasPopulatedTeam(state)` in `startRound.ts` (mirrors `startRound`'s own success condition, so the invariant "if prep opens, ROUND_START can't fail for lack of a team" holds); the `PREPARATION_OPEN` handler now refuses with `CANNOT_OPEN_PREP` ("Assign at least one player to a team first."). `Lobby.tsx` disables the Open-preparation button until a team is populated (server stays the authority; the gate is UX only).
  - **② "All experts can't start the round"** — not a separate bug: root-caused as ①. With players actually on a team, `startRound` force-promotes the rotation pick to Defuser even from an all-Expert roster, so it starts correctly; the failure Jay hit was the unassigned-roster case ① now blocks at the source.
  - **③ No exit from preparation** — added `PREPARATION_CANCEL` (client→server event) + facilitator-only handler + pure `cancelPreparation.ts` (`preparation → lobby`, decrements `roundNumber` as the exact inverse of `openPreparation`, so cancel+reopen doesn't skip a round number / 8.2 seed). `Preparation.tsx` gains a "Back to lobby" control next to Start.
  - **④ Refresh starts a new session (old one unreachable)** — confirmed *expected* for V1, already deferred (`deferred-work.md`: `players` keyed by the ephemeral `socket.id`; the session-reattach story introduces a durable player id). No change.
  - Tests added: pure `cancelPreparation` + `hasPopulatedTeam` suites; `PREPARATION_CANCEL` handler block; prep-open no-team guard test; the three existing prep-open handler tests updated for the new populated-team precondition (all-sockets broadcast drain to avoid stale-snapshot races). Gates re-run green: tsc 0; shared 53 / client 147 / server 219; client build OK.
  - Deferred from this session (`deferred-work.md`, "manual verification of story 8-3"): the **lobby role-pick vs rotation authority** design tension → **Story 8.6** (rotation-advance mechanics land there); and `cancelPreparation`'s hard-coded return-to-`lobby` to be revisited when `between-rounds → preparation` becomes reachable (8.5/8.6).
...
Post-verification fixes (commit `e921310`):
- apps/server/src/session/cancelPreparation.ts (created — pure preparation → lobby transition)
- apps/server/src/session/__tests__/cancelPreparation.test.ts (created — 6 tests)
- apps/server/src/session/startRound.ts (modified — exported pure `hasPopulatedTeam`)
- apps/server/src/session/__tests__/startRound.test.ts (modified — +3 `hasPopulatedTeam` tests)
- packages/shared/src/events/client-to-server.ts (modified — PREPARATION_CANCEL event)
- apps/server/src/handlers/sessionHandlers.ts (modified — prep-open population guard + PREPARATION_CANCEL handler + `hasPopulatedTeam` import)
- apps/server/src/handlers/__tests__/sessionHandlers.test.ts (modified — +PREPARATION_CANCEL block, +prep-open no-team guard test, prep-open tests updated for the populated-team precondition)
- apps/client/src/ui/Lobby.tsx (modified — Open-preparation button disabled until a team is populated)
...
- 2026-06-13: AC-6 interactive verification completed by Jay (three sessions against the Docker/Caddy prod stack) — confirmed. Three preparation-flow defects surfaced during the session were fixed (commit `e921310`): prep-open now requires a populated team (`hasPopulatedTeam` guard + `CANNOT_OPEN_PREP` + Lobby button gating); new `PREPARATION_CANCEL` provides a Back-to-lobby exit (pure `cancelPreparation`, inverse of `openPreparation`); the "all-experts can't start" report was root-caused as the unassigned-roster case the prep guard now blocks. Refresh-loses-session confirmed expected (deferred — socket.id identity). Role-pick-vs-rotation design tension deferred to Story 8.6. Re-verified live; gates green (tsc 0; shared 53 / client 147 / server 219; build).
